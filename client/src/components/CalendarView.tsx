...
export const CalendarView: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const l = recipes.filter(
      (r) => r.category === 'Үндсэн хоол' || r.category === 'Салат ба Хөнгөн зууш'
    );
    const d = recipes.filter(
      (r) => r.category === 'Шөл ба Бүлээн хоол' || r.category === 'Үндсэн хоол'
    );
    setSchedule(
      weekDays.map((w, idx) => ({
        dayId: w.day,
  ...
  const ingredientStatus = useMemo(() => {
    ...
    activeRecipe.ingredients.forEach((ing) => {
      ...
      if (hasItem) {
        ...
        missing.push({
          name: ing,
          // A placeholder 3,500₮ used to be quoted for ingredients the shop
          // does not carry, so the shopping list priced things nobody could buy.
          estimatedPrice: product?.pricePerUnit ?? null,
        ...
        <>
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            >
              <RotateCw
                size={14}
                className={`text-mango-ink ${isRegenerating ? 'animate-spin' : ''}`}
              />
              <span>
                {isRegenerating ? t('calendar_planning') : t('calendar_regenerateWithAi')}
              </span>
            </button>
          </header>
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
              ...
              <span className="text-xs font-bold text-gray-400">
                {t('calendar_selectedDay', { n: activeDaySchedule.dayLabel })}
              </span>
            </div>
              ...
              {[
                {
                  ...
                  label: t('calendar_breakfast'),
                  icon: Sun,
                  recipe: activeDaySchedule.breakfast,
                },
                {
                  id: 'lunch' as MealType,
                  label: t('calendar_lunch'),
                  icon: SunMedium,
                  recipe: activeDaySchedule.lunch,
                },
                {
                  id: 'dinner' as MealType,
                  label: t('calendar_dinner'),
                  icon: Moon,
                  recipe: activeDaySchedule.dinner,
                },
              ].map((m) => {
                const Icon = m.icon;
            ...
            <div className="h-48 sm:h-64 relative overflow-hidden group">
              ...
              <div className="absolute top-3 left-3 sm:top-4 sm:left-4 flex flex-wrap items-center gap-2 pr-24">
                ...
                <div className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-xl text-xs font-bold text-white flex items-center gap-1.5 border border-white/10">
                  ...
                  {formatDifficulty(activeRecipe.difficulty, t)}
                </div>
              </div>
            ...
            <div className="p-4 sm:p-5 space-y-5">
              ...
              <div className="grid grid-cols-4 gap-2 bg-pestle-bg p-3 rounded-2xl border border-pestle-border/60 text-center">
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-black text-mango-ink">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-black text-pestle-text">
                ...
                {ingredientStatus.available.length > 0 && (
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-extrabold text-mint-ink uppercase tracking-wider flex items-center gap-1">
                      ...
                      {t('calendar_availableIngredients', { n: ingredientStatus.available.length })}
                    </p>
                ...
                {ingredientStatus.missing.length > 0 && (
                  <div className="space-y-2 pt-1">
                    <p className="text-[10px] font-extrabold text-amber-700 dark:text-amber-400 uppercase tracking-wider flex items-center gap-1">
                      <AlertCircle size={12} />{' '}
                      {t('calendar_missingIngredients', { n: ingredientStatus.missing.length })}
                    </p>
                    ...
                    <div className="space-y-2 bg-pestle-bg p-3 rounded-2xl border border-pestle-border/60">
                      ...
                      {ingredientStatus.missing.some((item) => item.product) && (
                        <button
