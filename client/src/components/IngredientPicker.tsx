...
import { MAX_INGREDIENT_IMAGE_BYTES } from './EditIngredientModal';
...
export const IngredientPicker: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  // Real label↔control association (was visual adjacency only).
  const uid = useId();
  ...
  const handleAddCustom = async (e: React.FormEvent) => {
    ...
    // Upload to Storage rather than storing the whole photo as a base64 data
    // URL in the inventory row.
    let imageUrl = customImageBase64 || getIngredientImageUrl(customName) || undefined;
    if (customImageFile) {
      const uploaded = await uploadImage(customImageFile, 'ingredients');
  ...
  return (
    ...
    >
      <m.div
        ...
        className="bg-pestle-card border border-pestle-border w-full max-w-2xl rounded-t-[32px] sm:rounded-[32px] p-5 sm:p-6 max-h-[92dvh] flex flex-col shadow-2xl overflow-hidden"
      >
        ...
        <div className="flex justify-between items-start gap-3 pb-4 border-b border-pestle-border/60 shrink-0">
          <div className="min-w-0">
            <h2 className="text-lg sm:text-xl font-extrabold text-pestle-text">
              {isCustomMode
          ...
          <button
            onClick={onClose}
            aria-label={t('close')}
            className="w-9 h-9 shrink-0 bg-pestle-bg border border-pestle-border rounded-full flex items-center justify-center text-gray-400 hover:text-pestle-text transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        {/* Content Body */}
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain py-4 pb-sheet-safe sm:pb-4 space-y-4">
            ...
            <>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  aria-label={t('picker_addManually')}
                  title={t('picker_addManually')}
                  className="bg-mango text-white border border-mango/30 px-3 sm:px-3.5 py-2.5 rounded-xl text-xs font-bold hover:bg-mango/90 transition-colors flex items-center gap-1.5 whitespace-nowrap shadow-md shadow-mango/20 shrink-0"
                >
                  <Plus size={16} className="shrink-0" />
                  {/* Label drops below sm so it never squeezes the search field
                      down to a few characters on a phone. */}
                  <span className="hidden sm:inline">{t('picker_addManually')}</span>
                </button>
              </div>
          ...
          {isCustomMode && (
            <form onSubmit={handleAddCustom} className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                {/* A caption, not a <label>: the real control is the dropzone
                    below — the file input itself is visually hidden. */}
                <p className="text-xs font-bold text-pestle-text flex items-center justify-between">
                ...
                <div
                  role="button"
                  tabIndex={0}
                  aria-label={t('picker_ingredientImageOptional')}
                ...
                {imageError && (
                  <p className="text-xs font-semibold text-red-500 bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2 mt-2">
                    {imageError}
                  </p>
                )}
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {saving ? t('edit_saving') : t('picker_addToFridge')}
                </button>
              </div>
            </form>
          )}
